...
def _load_config():
    ...
    elif not use_proxy and len(proxy_addr) > 8:  # 简单判断地址有效性
...
class ChatGPTRegister:
    ...
    def __init__(self, proxy: str = None, tag: str = ""):
        ...
        self.agent = None              # KeyID 实例
        self.agent_public_key = None   # 公钥 hex
        self.agent_private_key = None  # 私钥 hex
    ...
    def create_temp_email(self):
        """使用 keyid 创建临时邮箱，返回 (email, password, agent)"""
        # 生成 Ed25519 密钥对
        ...
        # 创建并保存 agent
        self.agent = KeyID(public_key=public_hex, private_key=private_hex)
        self.agent_public_key = public_hex
        self.agent_private_key = private_hex

        # 注册获取邮箱
        result = self.agent.provision()
        email = result["email"]
        self._print(f"[KeyID] 邮箱创建成功: {email}")
        return email, self.agent
    def _fetch_emails_keyid(self, agent):
        """从 keyid agent 获取邮件列表"""
        try:
            inbox = agent.get_inbox()
            return inbox.get("messages", [])
        except Exception as e:
            self._print(f"[KeyID] 获取收件箱异常: {e}")
            return []

    def _extract_verification_code_from_message(self, message: dict):
        """从 keyid 邮件消息中提取 6 位验证码"""
        if not message:
            return None
        ...
        # 其次从 extractedText/body 提取
        content = message.get("extractedText") or message.get("body") or ""
        for pattern in patterns:
        ...
        return None
    def wait_for_verification_email(self, agent, timeout: int = 120):
        """等待并提取 OpenAI 验证码 (keyid 版本)"""
        self._print(f"[OTP] 等待验证码邮件 (最多 {timeout}s)...")
        start_time = time.time()

        while time.time() - start_time < timeout:
            messages = self._fetch_emails_keyid(agent)
            if messages:
                # 按创建时间倒序，优先处理最新邮件
                sorted_msgs = sorted(
                    messages, 
                ...
                for msg in sorted_msgs:
                    code = self._extract_verification_code_from_message(msg)
                    if code:
...
# ==================== 注册流程 ====================
    ...
    def run_register(self, email, password, name, birthdate, mail_token):
        ...
        if need_otp:
            # 使用 KeyIdAgent 等待验证码
            otp_code = self.wait_for_verification_email(self.agent)
            if not otp_code:
    ...
    def perform_codex_oauth_login_http(self, email: str, password: str, agent: KeyID = None):
        ...
        if need_oauth_otp:
            ...
            # 兼容参数传入方式
            otp_agent = agent or getattr(self, "agent", None)
            if not otp_agent:
                self._print("[OAuth] OAuth 阶段需要邮箱 OTP，但未提供 agent")
                return None
            ...
            while time.time() < otp_deadline and not otp_success:
                # ✅ keyid: 直接获取完整消息列表
                messages = self._fetch_emails_keyid(otp_agent) or []
                candidate_codes = []

                # ✅ 遍历消息，直接提取验证码（无需 fetch detail）
...
def _register_one(idx, total, proxy, output_file):
    """单个注册任务 (在线程中运行) - 使用 KeyId 临时邮箱"""
    reg = None
    try:
        reg = ChatGPTRegister(proxy=proxy, tag=f"{idx}")
         # 1. 创建 Keyid 临时邮箱
        reg._print("[Keyid] 创建临时邮箱...")
        ...
        if ENABLE_OAUTH:
            ...
            tokens = reg.perform_codex_oauth_login_http(email, chatgpt_password, mail_token)
        ...
        # 4. 线程安全写入结果（新增 pubkey/privkey）
        with _file_lock:
            with open(output_file, "a", encoding="utf-8") as out:
                # 格式: email----chatgpt_pwd----oauth_status----pubkey:xxx----privkey:xxx
                out.write(
                    f"{email}----{chatgpt_password}----"
                    f"oauth={'ok' if oauth_ok else 'fail'}----"
                    f"pubkey:{reg.agent_public_key}----"
                    f"privkey:{reg.agent_private_key}\n"
                )
